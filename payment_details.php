...
<html>
    ...
    if (paymentMode === "credit card") {
        paymentForm.action = "payment.php";
    } else if (paymentMode === "esewa") {
        paymentForm.action = "esewa.php";
    } else {
        // Set the default action if a different payment mode is selected
        paymentForm.action = "payment_details_form_handler.php";
    }
}
		</script>
		...
		<form id="paymentForm" action="payment_details_form_handler.php" method="post">
			...
			<table cellpadding="5" style='margin-left: 50px'>
				...
				<tr>
					...
					<td class="fix_table"><i class="fa fa-desktop" aria-hidden="true"></i> esewa <input type="radio" name="payment_mode" value="esewa"></td>
				</tr>
			</table>
			<br>
			<input type="submit" value="Pay Now" name="Pay_Now" onClick="redirectToPayment()">
		</form>
		<!-- 
•	Booking_Status
•	Payment_ID -->
